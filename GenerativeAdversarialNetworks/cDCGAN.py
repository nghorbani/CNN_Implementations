...
def create_gan_G(z, labels, is_training, Cout=1, trainable=True, reuse=False, networktype='ganG'):
    ...
    with tf.variable_scope(networktype, reuse=reuse):
        ...
        Gz = dense(z, is_training, Cout=4 * 4 * 256, trainable=trainable, act='reLu', norm='batchnorm', name='dense1')
        ...
        Gz = deconv(Gz, is_training, kernel_w=4, stride=1, Cout=Cout, trainable=trainable, act=None, norm=None, name='deconv3')  # 28
        Gz = tf.nn.sigmoid(Gz)
    return Gz
