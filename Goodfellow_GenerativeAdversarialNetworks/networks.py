...
def create_gan_G(z, labels, is_training, Cout=1, trainable=True, reuse=False, networktype='ganG'):
    ...
    with tf.variable_scope(networktype, reuse=reuse):
        ...
        Gz = dense(z,is_training, Dout=4 * 4 * 1024, act='ReLu', useBN=True,name='dense1')
        Gz = tf.reshape(Gz, shape=[batch_size, 4, 4, 1024])  # 4
...
def create_gan_D(xz, labels, is_training, trainable=True, reuse=False, networktype='ganD'):
    with tf.variable_scope(networktype, reuse=reuse):
        ...
        #Dxz = tf.reshape(Dxz, shape=[batch_size, 4 * 4 * 1024])
        #Dxz = dense(Dxz,is_training, Dout=10, act='lrelu', useBN=True, name='dense1')
        
        Dxz = conv(Dxz,is_training, kernel=2, stride=2, Cout=1024, trainable=trainable,act='lrelu', useBN=True,  name='conv3')  # 2
        Dxz = conv(Dxz,is_training, kernel=2, stride=2, Cout=10, trainable=trainable,act='lrelu', useBN=True,  name='conv4')  # 2
        #Dxz = tf.nn.max_pool(Dxz,[1,2,2,1],[1,2,2,1],padding='VALID')        
